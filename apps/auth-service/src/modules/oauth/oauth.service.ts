  ...
  async handleGoogleCallback(code: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new BadRequestException(err.error_description || 'Failed to exchange authorization code.');
      }
      tokenData = await response.json();
    } catch (err: any) {
      if (err instanceof BadRequestException) throw err;
      throw new InternalServerErrorException('Error communicating with Google token service.');
    }
    ...
    try {
      ...
      if (err instanceof BadRequestException) throw err;
      throw new InternalServerErrorException('Error fetching Google user profile.');
    }
