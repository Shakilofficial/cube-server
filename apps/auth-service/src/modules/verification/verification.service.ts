...
import { MAX_ATTEMPTS, MAX_RESEND } from "./utils/verification.utils";
import { hashSHA256 } from "../../core/utils/crypto.utils";
...
export class VerificationService {
  ...
  async resendVerification(
    userId: string,
  ): Promise<{ message: string; _devCode?: string }> {
    const user = await this.prisma.user.findUnique({ where: { id: userId } });
    if (!user) throw new NotFoundException("User not found.");
    if (user.status === "ACTIVE") {
    ...
    const codeHash = hashSHA256(newCode);
