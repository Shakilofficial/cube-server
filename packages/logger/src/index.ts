...
export function logStartupBanner(serviceName: string, details: Record<string, any>) {
  const width = 60;
  ...
  const titleLine = ' '.repeat(padLength) + titleText + ' '.repeat(width - 2 - padLength - titleText.length);
  ...
  for (const [key, val] of Object.entries(details)) {
    ...
    if (typeof val === 'boolean') {
      statusStr = val ? '\x1b[1;32m✔ CONNECTED\x1b[0;36m' : '\x1b[1;31m✘ OFFLINE\x1b[0;36m';
    } else {
    ...
    // Key-value spacing
    const keyPart = `  ${key}: `;
    const cleanVal = typeof val === 'boolean' ? (val ? '✔ CONNECTED' : '✘ OFFLINE') : String(val);
    const lineContentLength = keyPart.length + cleanVal.length;
    const padding = ' '.repeat(Math.max(0, width - 4 - lineContentLength));
    
    console.log(`\x1b[36m│\x1b[0m${keyPart}${statusStr}${padding}\x1b[36m│\x1b[0m`);
  }
...
export async function checkTcpConnection(urlStr: string, defaultPort: number): Promise<boolean> {
  return new Promise((resolve) => {
    try {
      ...
      const sanitizedUrl = urlStr.replace(/^amqp/i, 'http').replace(/^redis/i, 'http');
